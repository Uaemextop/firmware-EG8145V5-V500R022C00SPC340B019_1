...
type_word_v5="COMMON COMMON2 EUCOMMON EUCOMMON2 NACOMMON NACOMMON2 COMMONEBG COMMONEBG2 EUCOMMONEBG EUCOMMONEBG2 EUCOMMONEBG4 NACOMMONEBG NACOMMONEBG2 CLOSETELNET CLOSETELNETEBG CLOSETELNETEBG2 CHINA CHINAEBG CHINA4 CHINAEGB4"
...
HW_Script_SetDatToFile()
{
	...
	do
		...
		then
			...
			HW_Script_Cfg_Add_Clone $var_default_ctree $var_COMMONV5_Firewall /etc/wap/hw_firewall_v5.xml
    ...
    if [ $var_support_secure_feature = 1 ]; then
        if [ "$var_ebg_flag" != "1" ]; then
            ...
            HW_Script_Cfgtool_Set $var_default_ctree $var_node_admin_web_pwd "Password" $var_admin_pwd

            #设置管理员用户出厂密码，没有就新增
            cfgtool gettofile $var_default_ctree $var_node_admin_web_pwd "FactoryPassword"
            if [ 0 -eq $? ]
            then
                cfgtool set $var_default_ctree $var_node_admin_web_pwd "FactoryPassword" $var_admin_pwd
            else
                cfgtool add $var_default_ctree $var_node_admin_web_pwd "FactoryPassword" $var_admin_pwd
            fi
        fi

		cfgtool gettofile $var_default_ctree $var_node_cli_olt "ModifyPWDFlag"
		if [ 0 -eq $? ]
		then
			cfgtool set $var_default_ctree $var_node_cli_olt "ModifyPWDFlag" $cli_olt_modifyFlag
		fi
	fi
