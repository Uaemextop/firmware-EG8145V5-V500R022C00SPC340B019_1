...
# 写入两个SSID的recover脚本，该脚本通过读取 /var/customizepara.txt 
...
var_chipid=""
var_chipidsupport=`getboardinfo 0x00000083`
...
{
	read -r var_specsn var_userPwd var_ssid var_wpa var_chipid < $var_customize_file
	if [ 0 -ne $? ]
	then
...
HW_Script_SetDatToFile()
{
	...
	var_node_awifi_ssid=InternetGatewayDevice.LANDevice.LANDeviceInstance.1.WLANConfiguration.WLANConfigurationInstance.3
	...
	if [ 0 -eq $? ]
	then
		cfgtool set $var_default_ctree $var_node_ssid2 SSID "iTV-$var_para"
		if [ 0 -ne $? ]
		then
...
Script_SetChipId()
{
    ...
    var_node_chipid=InternetGatewayDevice.DeviceInfo

# set chipId
	cfgtool set $var_default_ctree $var_node_chipid CheckKey $var_chipid
	if [ 0 -ne $? ]
	then
		echo "Failed to set chipId!"
		return 1
	fi

	if [ "$var_chipidsupport" == "" ]; then
		echo "obj.id = \"0x00000083\" ; obj.value = \"$var_chipid\";" >> $var_boardinfo_dup
	fi

    echo $var_boardinfo_dup | xargs sed 's/obj.id = \"0x00000083\" ; obj.value = \"[A-Za-z0-9-]*\"/obj.id = \"0x00000083\" ; obj.value = \"'$var_chipid'\"/g' -i
	if [ 0 -ne $? ]
	then
		echo "dup boardinfo set failed!"
		return 1
