...
{
	read -r var_web_pwd_user var_web_pwd_admin var_ssid var_wpa var_ssid2 var_wpa2 < $var_customize_file
	if [ 0 -ne $? ]
	then
...
HW_Script_SetDatToFile()
{
	...
	cfgtool set $var_default_ctree_var $var_node_ssid5g SSID $var_ssid2
	if [ 0 -ne $? ]
	then
	...
	# set wpa password
	cfgtool set $var_default_ctree_var $var_node_wpa_pwd5g PreSharedKey $var_wpa2
	if [ 0 -ne $? ]
	then
	    echo "Failed to set common ssid wap5g password!"
	    return 1
	fi
