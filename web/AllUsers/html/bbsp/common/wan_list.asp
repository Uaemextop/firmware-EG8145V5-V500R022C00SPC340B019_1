...
function MakeWanName_New(wan) {
    ...
    if (((CfgModeWord.toUpperCase() == "AIS") || (CfgModeWord.toUpperCase() == "MOLDTEL2")) && wan.Name != '') {
        return wan.Name;
    }

    if ((CfgModeWord.toUpperCase() == "ETB") && (wan.Name != '')) {
        return wan.Name;
    }
    ...
    this.WanListOverlay = function(WanList, WanStats) {
        ...
        for (var i = 0; i < WanStats.length - 1; i++) {
            for (var j = 0; j < WanList.length; j++) {
                ...
                WanList[j].X_HW_UpstreamBitrate = WanStats[i].X_HW_UpstreamBitrate;
                WanList[j].X_HW_DownstreamBitrate = WanStats[i].X_HW_DownstreamBitrate;
            }
        }
    }
    this.WanListOverlayAll = function(WanList) {
