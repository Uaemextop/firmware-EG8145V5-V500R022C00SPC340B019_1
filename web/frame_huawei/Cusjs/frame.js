...
var Frame = {
    ...
    getRemoteData : function() {
	  $.ajax({
		...
		$.ajax({
            ...
            async : false,
            cache : false,
            url : "../html/ssmp/common/getPwdModifyFlag.asp",
            success : function(data) {
                jumptomodifypwd = data;
            }
        });
    },
	...
	this.setAppDesInfo(curLanguage);
    	...
    	if (this.menuItems.length > 0) {
			...
			{	
				...
				if ('ROSUNION' == CfgMode.toUpperCase()) /*ROS PK*/
				{
					pwdurl = "html/ssmp/accoutcfg/accountadmin.asp";			      
				}
				else
				{
				        pwdurl = "html/ssmp/accoutcfg/account.asp";
				}
    ...
    addMenuItems : function(element, menus, type) {
		...
		if(type == "main") {
			...
			for (var i = 0, len = menus.length; i < len; i++) {
				...
				if( menus[i].name == "System Tools" || menus[i].name == "Ferram. de Sis." || menus[i].name == "システムツール" || menus[i].name == "Herramientas del sistema")
				{
					SystemToolsNum = i;
				}
            ...
            for (var i = 0, len = menus.length; i < len; i++) {
				...
				if("Device Information" == menus[i].name || "Infor. do dispositivo" == menus[i].name || "デバイス情報" == menus[i].name || "Información del dispositivo" == menus[i].name)
				{
					posDeviceInfo = i;
				}

				if(menus[i].name == "Modify Login Password" || menus[i].name == "Alterar palavra-passe" || menus[i].name == "ログインパスワードの変更" || menus[i].name == "Modificar contraseña" )
				{
					PwdAspNum = i;
				}
			}
	...
	setCopyRightInfo : function(language) {
        if (language == "english") {
            ...
            $("#footerText").html("Copyright © ##.COPY_RIGHT_YEAR.## Huawei Technologies Co., Ltd. Todos os direitos reservados. ");
