...
CollectWifiLog()
{
	...
	RunCmd cat "/var/wifidlw"
	RunCmd cat "/var/wifipmlog"
}
...
CollectPktInfoMars()
{
	...
	chown -h 3024:2002 /var/wifi_collect_sign
...
CollectQCA()
{
	...
	do
		...
		if [ $? -eq 0 ];then
			...
			if [ $dev == ath0 ]||[ $dev == ath1 ]||[ $dev == ath2 ]||[ $dev == ath3 ]||[ $dev == ath4 ]||[ $dev == ath5 ]||[ $dev == ath6 ]||[ $dev == ath7 ];then

				ProtectDmesgExe iwpriv $dev txrx_stats 6
					
				ProtectDmesgExe iwpriv $dev txrx_stats 23
					
				ProtectDmesgExe iwpriv $dev txrx_stats 29				

				ProtectDmesgExe iwpriv $dev txrx_stats 30
									
				ProtectDmesgExe iwpriv $dev txrx_stats 31
									
				ProtectDmesgExe iwpriv $dev txrx_stats 258
					
				ProtectDmesgExe iwpriv $dev txrx_stats 259
			fi
...
CollectSolar()
{
	...
	solar_dev=$1
	solar_dev_list="wlan0-vap0 wlan0-vap1 wlan0-vap2 wlan0-vap3 wlan0-vap4 wlan0-vap5 wlan0-vap6 wlan0-vap7"
	wifi_chip_list="5G 5G_STA"

	if [ $solar_dev == wlan1 ]; then
		solar_dev_list="wlan1-vap0 wlan1-vap1 wlan1-vap2 wlan1-vap3 wlan1-vap4 wlan1-vap5 wlan1-vap6 wlan1-vap7"
		wifi_chip_list="2G 2G_STA"
	fi

	if [ $solar_dev == wlan0 ]; then
		RunCmd cat /var/run/hostapd-wlan0.conf
		RunCmd cat /var/hostapd_wlan0.log | grep -v psk | grep -v key | grep -v "_pin=" | grep -v "PIN" \
			| grep -v "passphrase" | grep -v "secret" | grep -v "uuid" | grep -v pairwise | grep -v cipher | grep -v password
	fi

	if [ $solar_dev == wlan1 ]; then
		RunCmd cat /var/run/hostapd-wlan1.conf
		RunCmd cat /var/hostapd_wlan1.log | grep -v psk | grep -v key | grep -v "_pin=" | grep -v "PIN" \
			| grep -v "passphrase" | grep -v "secret" | grep -v "uuid" | grep -v pairwise | grep -v cipher | grep -v password
	fi

	for dev in $solar_dev_list
	do
		ifconfig $dev | grep UP
		#$? mains the return state of last command
		if [ $? -eq 0 ]
		then
			RunCmd cat /proc/net/rtk_wifi6/$dev/int_logs
			RunCmd cat /proc/net/rtk_wifi6/$dev/tx_logs
			RunCmd cat /proc/net/rtk_wifi6/$dev/rx_logs
		fi
	done

	RunCmd cat /proc/net/rtk_wifi6/ver_info
	RunCmd cat /proc/net/rtk_wifi6/$solar_dev/ap_info
	RunCmd cat /proc/net/rtk_wifi6/$solar_dev/chan
	RunCmd cat /proc/net/rtk_wifi6/$solar_dev/all_sta_info | grep -v psk | grep -v key | grep -v "_pin=" | grep -v "PIN" \
		| grep -v "passphrase" | grep -v "secret" | grep -v "uuid" | grep -v pairwise | grep -v cipher | grep -v password
	RunCmd cat /proc/net/rtk_wifi6/$solar_dev/trx_info
	RunCmd cat /proc/net/rtk_wifi6/$solar_dev/tx_logs
	RunCmd cat /proc/net/rtk_wifi6/$solar_dev/rx_logs
	RunCmd cat /proc/net/rtk_wifi6/$solar_dev/bd_info
	RunCmd cat /proc/net/rtk_wifi6/$solar_dev/rf_para_info
	RunCmd iwpriv $solar_dev phl_test debug,dump_rx; dmesg -c

	CollectWifiLog $wifi_chip_list
	CollectPktInfoCommon $solar_dev_list
}
