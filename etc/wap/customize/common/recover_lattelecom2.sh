...
HW_Script_SetDatToFile()
{
	...
	if [ 0 -ne $? ]
	then
	    echo "Failed to set common ssid wap2 password!"
	    return 1
	fi
	...
	if [ 0 -ne $? ]
	then
	    echo "Failed to set common ssid3 name!"
	    return 1
	fi

	# set wpa password
	cfgtool set $var_default_ctree $var_node_wpa_pwd3 PreSharedKey $var_wpa
	if [ 0 -ne $? ]
	then
	    echo "Failed to set common ssid wap3 password!"
	    return 1
	fi
	...
	if [ 0 -ne $? ]
	then
	    echo "Failed to set common ssid4 name!"
	    return 1
	fi

	# set wpa password
	cfgtool set $var_default_ctree $var_node_wpa_pwd4 PreSharedKey $var_wpa
	if [ 0 -ne $? ]
	then
	    echo "Failed to set common ssid wap4 password!"
	    return 1
	fi
	
	# set ssid 
	cfgtool set $var_default_ctree $var_node_ssid5 SSID $var_ssid2
	if [ 0 -ne $? ]
	then
	...
	# set wpa password
	cfgtool set $var_default_ctree $var_node_wpa_pwd5 PreSharedKey $var_wpa2
	if [ 0 -ne $? ]
	then
