...
HW_Script_SetDatToFile()
{
	...
	var_node_pppoe=InternetGatewayDevice.WANDevice.WANDeviceInstance.1.WANConnectionDevice.WANConnectionDeviceInstance.2.WANPPPConnection.WANPPPConnectionInstance.1
	
	var_sn=$(cat $var_boardinfo_dup | grep 0x00000002 | cut -d '"' -f 4)
	if [ ${#var_sn} -eq 0 ]
	then
		echo "Please set SN first!"
		return 1
	fi

	var_sn_last6=${var_sn:$(expr ${#var_sn} - 6)}
	if [ ${#var_sn_last6} -ne 6 ]
	then
		echo "Failed to get last 6 characters of SN!"
		return 1
	fi
	
	var_ppp_user=$(head -n 4 /proc/wap_proc/wap_log | grep SerialNumber | cut -d: -f2 | tr -d "; \r\n")
	if [ ${#var_ppp_user} -eq 0 ]
	then
		echo "Failed to get pppoe username!"
		return 1
	fi

	# set pppoe username 
	cfgtool set $var_default_ctree_var $var_node_pppoe Username $var_pppoe_unm
	if [ 0 -ne $? ]
	then
		echo "Failed to set pppoe Username!"
		return 1
	fi
	
	# set pppoe password 
	cfgtool set $var_default_ctree_var $var_node_pppoe Password $var_pppoe_pwd
	if [ 0 -ne $? ]
	then
		echo "Failed to set common Password!"
		return 1
	fi
